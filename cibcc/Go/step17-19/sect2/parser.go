...
func push_var(name []rune, ty *Type) *Var {
	va := &Var{name: name, ty: ty}
...
func read_func_param() *VarList {
	ty := basetype()
	name := expect_ident()
	return &VarList{va: push_var(name, ty)}
}

func declaration() *Node {
	ty := basetype()
	va := push_var(expect_ident(), ty)
