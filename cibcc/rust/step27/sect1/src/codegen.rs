...
fn emit_data(prog: &Program, output: &mut String) {
	...
	while let Some(vl) = g_ptr {
		...
		if var.contents.is_empty() {
			output.push_str(&format!("  .zero {}\n", var.ty.size()));
		} else {
			for i in 0..var.cont_len {
				let b = var.contents.as_bytes()[i as usize];
...
fn gen(node: &Node, prog: &Program, f: &Function, output: &mut String, labelseq: &mut i64) {
	match &node.kind {
		...
		NodeKind::Assign => {
			gen_lvar(node.lhs.as_ref().unwrap(), prog, f, output, labelseq);
		...
		NodeKind::Block | NodeKind::StmtExpr => {
			...
			while let Some(n) = cur {
				...
				if node.kind == NodeKind::StmtExpr && n.next.is_none() {
...
fn gen_addr(node: &Node, prog: &Program, f: &Function, output: &mut String, labelseq: &mut i64) {
	match &node.kind {
		NodeKind::Var => {
			if let Some(ref node_var) = node.var {
				let mut found = false;

				let mut cur = f.locals.as_deref();
				while let Some(vl) = cur {
					if vl.var.name == node_var.name {
						writeln!(output, "  lea rax, [rbp-{}]", vl.var.offset).unwrap();
						found = true;
						break;
					}
					cur = vl.next.as_deref();
				}
				if !found {
					let mut g_cur = prog.globals.as_ref();
					while let Some(vl) = g_cur {
						if vl.var.name == node_var.name {
							writeln!(output, "  lea rax, {}[rip]", vl.var.name).unwrap();
							found = true;
							break;
						}
						g_cur = vl.next.as_deref();
					}
				}

				if !found {
					panic!("undefined variable: {}", node_var.name);
				}
			}
			output.push_str("  push rax\n");
		}
...
fn gen_lvar(node: &Node, prog: &Program, f: &Function, output: &mut String, labelseq: &mut i64) {
