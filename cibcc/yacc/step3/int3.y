...
xpow      : xpow DEMI                   { push($2); }
          ...
          | DIG '/' DIG '*' VAR POW '(' DEMI DIG '/' DIG ')'
                                                 { xpow($1, $3, $8, $9, $11); }
          ;
xepow     : xepow DEMI                  { push($2); }
          | VAR EXP POW VAR             { xepow(1, 1, 1, a, 1); }
          | VAR EXP POW DIG VAR         { xepow(1, 1, 1, a, $4); }
          | VAR POW DIG EXP POW VAR     { xepow(1, 1, $3, a, 1); }
          | VAR POW DIG EXP POW DIG VAR   { xepow(1, 1, $3, a, $6); }
          | VAR EXP POW DEMI VAR        { xepow(1, 1, 1, $4, 1); }
          | VAR EXP POW DEMI DIG VAR    { xepow(1, 1, 1, $4, $5); }
          | VAR POW DIG EXP POW DEMI VAR  { xepow(1, 1, $3, $6, 1); }
          | VAR POW DIG EXP POW DEMI DIG VAR  { xepow(1, 1, $3, $6, $7); }
          | VAR EXP POW '(' DEMI VAR ')'  { xepow(1, 1, 1, $5, 1); }
          | VAR EXP POW '(' DEMI DIG VAR ')'  { xepow(1, 1, 1, $5, $6); }
          | VAR POW DIG EXP POW '(' DEMI DIG VAR ')'
	                                      { xepow(1, 1, $3, $7, $8); }
          ;
