...
class Var:
	def __init__(self, next=None, name=None, ty=None, offset=None):
		self.next = next
		self.ty = ty
		self.offset = offset
...
class Parser:
	...
	def read_func_param(self):
		vl = VarList()
		ty = self.basetype()
		vl.var = self.push_var(self.expect_ident(), ty)
		return vl

	def declaration(self):
		tok = self.tokens[self.pos]
		ty = self.basetype()
		var = self.push_var(self.expect_ident(), ty)
