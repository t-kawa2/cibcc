...
impl Parser {
	...
	fn push_var(&mut self, name: String, ty: Type) -> Var {
		let var = Var{
			name: name.clone(),
			ty: ty.clone(),
	...
	fn read_func_params(&mut self) -> Option<Box<VarList>> {
		...
		let var_body = self.push_var(name, ty.clone());
	...
	fn unary(&mut self) -> Box<Node> {
		...
		if self.consume("*") {
			...
			if let Some(ref lhs_ty) = node.lhs.as_ref().unwrap().ty {
				if let Some(ref base) = lhs_ty.base {
	...
	fn primary(&mut self) -> Box<Node> {
		...
		match tok.kind {
			...
			TokenKind::Ident(name) => {
				...
				let mut node = Node::new_var(var.clone());
