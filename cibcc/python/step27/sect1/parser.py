...
class Var:
	def __init__(self, name=None, ty=None, is_local=None, offset=None, contents=None, cont_len=None):
		self.name = name
		self.ty = ty
...
class Parser:
	...
	def consume_str(self):
		if self.tokens[self.pos].kind != 'STR':
			return None
		tok = self.tokens[self.pos]
		self.pos += 1
		return tok
	...
	def is_typename(self):
		return self.peek("char") or self.peek("int")

	def new_label(self):
		label = f".L.data.{self.label_cnt}"
		self.label_cnt += 1
		return label
	...
	def stmt_expr(self):
		...
		if cur.kind != 'EXPR_STMT':
			print("stmt expr returning void is not supported")
		cur = cur.lhs
		return node
	...
	def primary(self):
		...
		if tok := self.consume_str():
			ty = array_of(char_type(), tok.cont_len)
			var = self.push_var(self.new_label(), ty, False)
