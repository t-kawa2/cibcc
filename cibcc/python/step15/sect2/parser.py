class Node:
	def __init__(self, kind, lhs=None, rhs=None, cond=None, then=None, els=None, init=None, inc=None, body=None, funcname=None, args=None, val=None, next=None, var=None):
		self.kind = kind
		self.lhs = lhs
		self.rhs = rhs
		self.cond = cond
...
class Parser:
	...
	def read_func_params(self):
		if self.consume(")"):
			return None
		head = VarList()
		head.var = self.push_var(self.expect_ident())
		cur = head
		while not self.consume(")"):
			self.expect(",")
			cur.next = VarList()
			cur.next.var = self.push_var(self.expect_ident())
			cur = cur.next
		return head
