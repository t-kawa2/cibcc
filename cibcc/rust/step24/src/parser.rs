...
impl Parser {
	...
	fn push_var(&mut self, name: String, ty: Type, is_local: bool) -> Var {
		if is_local {
	...
	fn read_func_param(&mut self) -> Option<Box<VarList>> {
		...
		Some(Box::new(VarList::new(self.push_var(name, ty, true))))
	}
	...
	pub fn function(&mut self) -> Function {
		...
		let mut f = Function::new(name, params, Some(Box::new(head)), self.locals.take());
