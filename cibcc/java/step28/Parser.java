...
public class Parser {
	...
	private Var findVar(String str) {
		for (VarList vl = scope; vl != null; vl = vl.next) {
			Var va = vl.va;
	...
	private Node stmt_expr(Token tok) {
		VarList sc = scope;
		Node node = new Node(NodeKind.ND_STMT_EXPR, tok);
		Node head = null;
		Node cur = null;

		while (!consume("}")) {
			Node s = stmt();
			if (peek("}") != null && s.kind == NodeKind.ND_EXPR_STMT) {
				if (s.kind == NodeKind.ND_EXPR_STMT) {
					s = s.lhs;
				}
			}
			if (head == null) {
	...
	private Node primary() {
		...
		if (token.type == TokenType.TK_STR) {
			...
			Type ty = Type.array_of(Type.char_type(), curtok.cont_len);
