...
pub fn codegen(prog: Function) -> String {
	...
	while let Some(f) = curr_fn {
		writeln!(output, ".globl {}", f.name).unwrap();
		...
		let mut cur_param = f.params;
		let mut i = 0;

		while let Some(param_list) = cur_param {
			if let Some(ref var) = param_list.var {
				writeln!(output, "  mov [rbp -  {}], {}", var.offset, arg_regs[i]).unwrap();
				i+= 1;
			}
			cur_param = param_list.next;
		}

		let mut curr_node = f.node;
...
fn gen(node: &Node, output: &mut String, labelseq: &mut i64) {
	match &node.kind {
		...
		NodeKind::Return => {
			...
			output.push_str("  pop rax\n");
			writeln!(output, "  jmp .Lreturn.{}", node.funcname).unwrap();
		}
		...
		NodeKind::EQ  | NodeKind::NE  | NodeKind::LT  | NodeKind::LE  => {
			...
			match &node.kind {
				...
				NodeKind::Div => {output.push_str("  cqo\n");
