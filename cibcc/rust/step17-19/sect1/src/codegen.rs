...
fn gen(node: &Node, f: &Function, output: &mut String, labelseq: &mut i64) {
	match &node.kind {
		...
		NodeKind::EQ  | NodeKind::NE  | NodeKind::LT  | NodeKind::LE  => {
			...
			match &node.kind {
				NodeKind::Add => { if node.ty.as_ref().map_or(false, |t| t.kind == TypeKind::Ptr) {
								output.push_str("  imul rdi, 8\n");
							}
							output.push_str("  add rax, rdi\n");
						},
				NodeKind::Sub => { if node.ty.as_ref().map_or(false, |t| t.kind == TypeKind::Ptr) {
								output.push_str("  imul rdi, 8\n");
							}
