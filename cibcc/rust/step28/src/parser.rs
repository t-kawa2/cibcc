...
impl VarList {
	pub fn new(var: Rc<RefCell<Var>>) -> Self {
		Self {
			next: None,
			var,
		}
...
impl Node {
	...
	fn new_var(var: Rc<RefCell<Var>>) -> Box<Node> {
		Box::new(Node{
			kind: NodeKind::Var,
			lhs: None,
			rhs: None,
...
impl Parser {
	...
	fn find_var(&self, name: &str) -> Option<Rc<RefCell<Var>>> {
		let mut cur = &self.scope;
		while let Some(vl) = cur {
			if vl.var.borrow().name == name {
				return Some(Rc::clone(&vl.var));
			}
			cur = &vl.next;
		}
	...
	fn push_var(&mut self, name: String, ty: Type, is_local: bool, contents: String, cont_len: usize) -> Rc<RefCell<Var>> {
		...
		if is_local {
			self.locals = Some(Box::new(VarList{
				next: self.locals.take(),
				var: Rc::clone(&var),
			}));
			self.scope = Some(Box::new(VarList{
				next: self.scope.take(),
				var: Rc::clone(&var),
			}));
		} else {
			self.globals = Some(Box::new(VarList{
				next: self.globals.take(),
				var: Rc::clone(&var),
			}));
		}
		var
	}
	fn _read_expr_stmt(&mut self) -> Box<Node> {
	...
	fn declaration(&mut self) -> Node {
		...
		self.expect("=");
		let lhs = Node::new_var(Rc::clone(&var));
		let rhs = self.expr();
		self.expect(";");
	...
	pub fn function(&mut self) -> Function {
		...
		let mut offset = 0;
		{
			...
			while let Some(vl) = vl_ptr {
				let size = vl.var.borrow().ty.size() as i64;
				offset += size;
				offset = (offset +7) & !7;
				vl.var.borrow_mut().offset = offset;
		...
		}
		{
			...
			while let Some(p_vl) = p_ptr {
				...
				while let Some(l_vl) = l_ptr {
					if p_vl.var.borrow().name == l_vl.var.borrow().name {
						let l_offset = l_vl.var.borrow().offset;
						p_vl.var.borrow_mut().offset = l_offset;
						break;
					}
	...
	fn stmt(&mut self) -> Box<Node> {
		if self.consume("return") {
			...
			if !self.consume(";") {
				node.cond = Some(self.expr());
				self.expect(";");
			}
			if !self.consume(")") {
				node.inc = Some(Node::new_unary(NodeKind::ExprStmt, self.expr()));
				self.expect(")");
			}
			node.then = Some(self.stmt());
			return Box::new(node);
	...
	fn unary(&mut self) -> Box<Node> {
		...
		if self.consume("sizeof") {
			...
			return Node::new_num(size as i64);
		}
	...
	fn primary(&mut self) -> Box<Node> {
		...
		match tok.kind {
			...
			TokenKind::Ident(name) => {
				...
				node_inner.ty = Some(var.borrow().ty.clone());
				return node_inner;
			}
			TokenKind::Str => {
